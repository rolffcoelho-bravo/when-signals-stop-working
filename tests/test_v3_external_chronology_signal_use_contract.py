...
def test_parent_lock_and_frozen_determinations_are_exact() -> None:
    ...
    assert value["parent_lock_blob_sha"] == "0e35908c03e36d8caeb832a078ff0566ef4e2ea4"
...
def test_chronology_is_compiled_independently_before_overlay() -> None:
    ...
    assert value["construction_order"] == "chronology_registry_locked_before_model_output_overlay"
...
def test_chronology_is_not_treated_as_complete_ground_truth() -> None:
    ...
    assert value["event_alignment_evaluation"]["chronology_completeness_adjustment_required"] is True
    assert value["event_alignment_evaluation"]["classification_metrics_prohibited_without_complete_event_registry"] is True


def test_alignment_metrics_horizons_and_models_are_frozen() -> None:
    value = contract()["event_alignment_evaluation"]
    assert value["registered_models"] == ["MONOTONE_MECHANISM_SCORE_V1", "CAUSAL_GAUSSIAN_HMM_V1"]
...
def test_rsi_and_bollinger_remain_ineligible_for_rescue() -> None:
    ...
    assert frozen["BOLLINGER"]["eligibility"] == "INELIGIBLE_BASELINE_NOT_ESTABLISHED"
...
def test_required_outputs_and_subgate_sequence_are_frozen() -> None:
    ...
    assert [item["gate"] for item in value["subgate_sequence"]] == ["V3-4A", "V3-4B", "V3-4C", "V3-4D"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("event_alignment_evaluation", "threshold_selection_against_chronology_prohibited", False),
        ("signal_use_eligibility", "regime_conditioning_cannot_promote_signal", False),
        ("conditional_diagnostic_design", "signal_retraining_or_retuning_prohibited", False),
    ],
)
def test_contract_mutations_fail_closed(mutation: tuple[str, str, object]) -> None:
