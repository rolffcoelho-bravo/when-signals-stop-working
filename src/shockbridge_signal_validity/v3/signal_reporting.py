...
def build_reports(
    ...
    statuses: Mapping[str, pd.Series],
    ...
    expected_rows = len(data) * len(specs)
    feature_manifest = {
        ...
        "row_count_identity_verified": len(output) == expected_rows,
    ...
    coverage_report = {
        "schema_version": "v3.signal-coverage.v1",
        "source_rows": len(data),
        "signal_count": len(specs),
        "signals": coverage,
    ...
    validation = {
        "schema_version": "v3.signal-validation.v1",
        "valid": True,
        "gate": "V3-4",
        "required_columns_verified": list(REQUIRED_COLUMNS),
        "input_columns_accessed": list(REQUIRED_COLUMNS),
        "context_columns_accessed": list(context_columns),
