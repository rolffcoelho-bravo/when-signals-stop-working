...
try {
    ...
    Write-Host "3. REVALIDATING V3-5 FOUNDATION"
    .\RUN_V3_G5_FOUNDATION.ps1
    if ($LASTEXITCODE -ne 0) {
        throw "Gate V3-5 foundation revalidation failed."
    }

    Write-Host "4. RUNNING MATERIALIZATION TESTS"
    ...
    if (-not (Test-Path $SignalFeatures)) {
        Write-Host "5. REGENERATING LOCKED V3-4 RUNTIME SIGNAL EVIDENCE"
        ...
        Write-Host "5. USING EXISTING LOCKED V3-4 RUNTIME SIGNAL EVIDENCE"
    }

    Write-Host "6. MATERIALIZING REAL DEVELOPMENT-ONLY FOUNDATION"
    python scripts/run_v3_g5_materialization.py
    if ($LASTEXITCODE -ne 0) {
        throw "Gate V3-5 real foundation materialization failed."
    }

    Write-Host "7. VERIFYING COMPLETE MATERIALIZATION EVIDENCE"
